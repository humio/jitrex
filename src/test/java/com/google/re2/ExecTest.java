...
public class ExecTest {
  ...
  public void testExamplesInDocumentation() {
    Pattern re1 = Pattern.compile("co(.)a", Pattern.CASE_INSENSITIVE);
    assertEquals(Arrays.asList("Copa", "coba"), re1.findAll("Copacobana", 10));
    Pattern re2 = Pattern.compile("(?i-s)co(.)a");
    assertEquals(Arrays.asList("Copa", "coba"), re2.findAll("Copacobana", 10));
    Pattern re3 = Pattern.compile("(?i:co(.))a");
    assertEquals(Arrays.asList("Copa", "COBa"), re3.findAll("CopaCOBana", 10));
...
//    assertEquals(Arrays.asList("coba", "b"), Arrays.asList(x.get(1)));
  ...
  public void testRE2(String file) throws IOException {
    ...
    while ((line = r.readLine()) != null) {
      ...
      if ('A' <= first && first <= 'Z') {
        ...
        re = refull = null;
        try {
          re = Pattern.compile(q);
        } catch (Throwable e) { // (handle compiler panic too)
          if (String.valueOf(e.getMessage()).equals("error parsing regexp: invalid escape sequence: `\\C`")) {
            // We don't and likely never will support \C; keep going.
            continue;
          }
          System.err.format("%s:%d: compile %s: %s\n", file, lineno, q, e.getMessage());
          if (++nfail >= 100) {
            throw new AssertionError("stopping after " + nfail + " errors", e);
          }
          continue;
        }
        String full = "\\A(?:" + q + ")\\z";
        try {
          refull = Pattern.compile(full);
        } catch (Throwable e) { // (handle compiler panic too)
          // Fatal because q worked, so this should always work.
          throw new AssertionError(String.format("%s:%d: compile full %s: %s", file, lineno, full, e.getMessage()), e);
        }
        input = 0;
        ...
        for (int i = 0; i < fields.length; i++) { res.add(fields[i]); }
        if (res.size() != 4) {
        ...
        for (int i = 0; i < 4; ++i) {
          ...
          Pattern regexp = partial ? re : refull;
  ...
  private static final Pattern NOTAB = Pattern.compile("[^\t]+");
  ...
  private void testFowler(String file) throws IOException {
    ...
    while ((line = r.readLine()) != null) {
      ...
      for (char c : flag.toCharArray()) {
        ...
        int flags = Pattern.POSIX | RE2.CLASS_NL;
        switch (c) {
          default:
            continue;
          case 'E':
